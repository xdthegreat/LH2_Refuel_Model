...
LH2_consumed_warm_fill = zeros([1, length(Feed_temp_sweep_simOut)]);
LH2_in_AC_tank_warm_fill = zeros([1, length(Feed_temp_sweep_simOut)]);
frac_useful_LH2_warm_fill = zeros([1, length(Feed_temp_sweep_simOut)]);

LH2_consumed_cold_fill = zeros([1, length(Feed_temp_sweep_simOut)]);
LH2_in_AC_tank_cold_fill = zeros([1, length(Feed_temp_sweep_simOut)]);
frac_useful_LH2_cold_fill = zeros([1, length(Feed_temp_sweep_simOut)]);

time_warm_refuel = zeros([1, length(Feed_temp_sweep_simOut)]);
    ...
    for i = 1:length(Feed_temp_sweep_simOut)
        ...
        = multiple_sim_phase_parsing(Feed_temp_sweep_simOut(1, i), i);
        ...
        % error handling
            ...
            LH2_FEED_TEMP_VEC(i) = [0];

        end


    LH2_FEED_TEMP_VEC_copy = [];
    ...
    for i = 1:length(hose_insulation_sweep_simOut)
        if LH2_FEED_TEMP_VEC(i) ~= [0]
            LH2_FEED_TEMP_VEC_copy = [LH2_FEED_TEMP_VEC_copy, 
                LH2_FEED_TEMP_VEC(i)];
    ...
    LH2_FEED_TEMP_VEC = LH2_FEED_TEMP_VEC_copy;
